...
class KmerCodebook
{
  ...
  protected:
	...
	const EncodedFastaSequence::Index &dbIndex;

	// Index used to find prototype by name in the prototype database.
	const EncodedFastaSequence::Index &protoIndex;
  ...
  public:
	KmerCodebook(
		...
		uint charsPerWord,
		uint kmerLength,
		EncodedFastaSequence::Index &dbIndex,
		EncodedFastaSequence::Index &protoIndex,
		KmerIndex &kmerIndex,
		FILE *savedCodebook,
	...
	//---------------------------------------
	{
		...
		CopyKmerDataNonRecursive((int)Alphabet::WordsPerKmer(kmerLength, charsPerWord));
  ...
  private:
		...
		for (int i = 0; i < C; i++)
		{
			...
			if (cSize >= splitClusterThreshold)
			{
				...
				for (int j = 0; j < slices - 1; j++)
				{
					...
					vector<K> &newItems = newCluster->kmers;
	...
	static void gotoSymbol(const char *l, size_t &i, char symbol)
	{
		...
		if (!l[i])
			throw Exception(string("Expected '") + symbol + "'!", FileAndLine);
	};
		...
		pCluster &currentCluster)
	{
		...
		auto seqItem = dbIndex.find(l);

		if (seqItem == dbIndex.end())
		{
			throw Exception(string("sequence with Id ") + l + "cannot be found in database.", FileAndLine);
		}

		EncodedFastaSequence *seq = seqItem->second;
		uint offset = parseInt(l, i);

		Substring kmerText(seq->Sequence().c_str(), offset, kmerLength);
		...
		try
		{
			...
			{
				i++;
				Distance d = parseInt(l, i);
				kmer->DistanceFromPrototype(d);
			}

			currentCluster->Add(*kmer);
		}
...
#pragma omp parallel for schedule(dynamic, 10)
		for (int64_t i = 0; i < N; i++)
		{
			...
			Cluster *closest = FindNearestCluster(*kmer, dist);
			...
			if (threshold == 0 || dist <= threshold)
			{
				...
				closest->Add(*kmer);
				kmer->DistanceFromPrototype(dist);
