...
#pragma omp parallel for
		...
		*/
		...
		/*
		**	Serialize the position weight matrix to a stream.
		*/
		...
		/*
		**	Serialize the kmers, saving the sequence id, position, and length only.
		...
		*/
		...
		/*
		**	Serialize the cluster to a stream in text format.
		*/
		...
		/*
		**	Serialize a list of clusters to a stream in text format.
		*/
		...
		/*
		**	Deserialize a text-formatted cluster from a stream (if present and all OK)
		...
		*/
		...
		static P Deserialize(
			istream & in,
			const EncodedFastaSequence::Index & idx,
			const DistanceFunction & symbolCodeDist
		) {
			P result = 0;
			string inputLine;
			...
			for (size_t i = 0; i < kmerCount; i++) {
				...
				if (idx.find(id) != idx.end()) {
					...
					result->kmers.emplace_back(sequence, pos, rows);
				}
			}
		...
		/*
		**	Deserialize a collection of clusters from a stream (if present and all OK)
...
#pragma omp parallel for     // Create placeholders for any pre-existing clusters. 
...
#pragma omp critical
			...
			// Coelesce the items that were in the original set of clusters.
#pragma omp parallel for       
