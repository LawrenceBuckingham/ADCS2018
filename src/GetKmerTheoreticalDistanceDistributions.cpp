	...
	public:
		...
		*/
		static void Run( Args args ) {
			...
			EncodedFastaSequence::ReadSequences( db, parms.dbFile, parms.idIndex, -1, Alphabet::DNA(), parms.kmerLength, 1, 'a', EncodedFastaSequence::DefaultFactory );
			cerr << db.Length() << " sequences loaded from '" << parms.dbFile << "'" << endl;

			EncodedFastaSequence::Index idx( db.Items() );

			SimilarityMatrix * matrix = SimilarityMatrix::GetMatrix( parms.dist, parms.matrixId, parms.matrixFile, parms.isCaseSensitive );
			Histogram<char> symbolHistogram = FastaSequence::GetSymbolHistogram( db.Items() );
			TRACE;
			function<Distance( char, char )> symbolDistance = [matrix]( char x, char y ) { return matrix->Difference( x, y ); };
			TRACE;

			IntegerDistribution rawKmerDist = IntegerDistribution::GetKmerDistanceDistribution<char, Distance>( symbolHistogram, symbolDistance, parms.kmerLength );
			TRACE;
			...
			if ( distributionFile ) rawKmerDist.Print( *distributionFile, []( double x ) { stringstream s; s << setprecision( 17 ) << x; return s.str(); } );
			TRACE;
			...
			for ( uint n = 2; n <= 1 << 24; n *= 2 ) {
				...
				if ( distributionFile ) {
					...
					*distributionFile << "~~~~~\nMax absolute error between Sum of m minima of " << n << " distances and its normal approximation\n~~~~~\nm\tmaxError\n";
					...
					for ( uint m = 2; m <= 1024; m *= 2 ) {
						...
						( *distributionFile << "Average of minima:\t" << m << "\t" << "Normal approximation:\tmu" << "\t" << mu << "\t" << "sigma" << "\t" << sigma << "\t" << "maxError" << "\t" << maxError << "\n" ).flush();
				...
				if ( distributionFile ) ( *distributionFile << "\n" ).flush();
				TRACE;
			}
			...
			if ( aicFile ) delete aicFile;
			if ( distributionFile ) delete distributionFile;
			if ( paramFile ) delete paramFile;
		}
...
int main( int argc, char ** argv ) {
	...
	try {
		AdHoc::GetKmerTheoreticalDistanceDistributions::Run( arguments );
	}
	catch ( Exception ex ) {
